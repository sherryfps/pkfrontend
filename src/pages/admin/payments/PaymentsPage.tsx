...
import { IndianRupee, Search, CreditCard, Clock, CheckCircle2, XCircle } from 'lucide-react'
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[
          { label: 'Total Order Value', value: formatCurrency(totalValue), icon: IndianRupee, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-900/20' },
          { label: 'Total Received', value: formatCurrency(totalReceived), icon: CheckCircle2, color: 'text-green-600', bg: 'bg-green-50 dark:bg-green-900/20' },
          { label: 'Pending Collection', value: formatCurrency(totalPending), icon: Clock, color: 'text-amber-600', bg: 'bg-amber-50 dark:bg-amber-900/20' },
        ].map((card) => (
          <motion.div
            key={card.label}
                ...
                {filtered.map((o) => (
                  <tr key={o.id} className="hover:bg-gray-50/50 dark:hover:bg-gray-800/30 transition-colors">
                    <td className="px-5 py-3.5">
                      <span className="text-sm font-mono font-semibold text-primary-600">{o.orderNumber}</span>
                    </td>
                    <td className="px-5 py-3.5">
                      <p className="text-sm font-medium text-gray-900 dark:text-white">{o.customer?.name || 'N/A'}</p>
                      <p className="text-xs text-gray-400">{o.customer?.company || ''}</p>
                    </td>
                    <td className="px-5 py-3.5 text-right">
                      <span className="text-sm font-bold text-gray-900 dark:text-white">{formatCurrency(o.totalAmount)}</span>
                    </td>
                    <td className="px-5 py-3.5 text-right">
                      <span className="text-sm font-semibold text-green-600">{formatCurrency(o.paidAmount)}</span>
                    </td>
                    <td className="px-5 py-3.5 text-right">
                      <span className={`text-sm font-semibold ${o.balanceAmount > 0 ? 'text-amber-600' : 'text-gray-400'}`}>
                        {formatCurrency(o.balanceAmount)}
                      </span>
                    </td>
                    <td className="px-5 py-3.5">
                      <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-semibold ${paymentStatusColors[o.paymentStatus] || 'bg-gray-100 text-gray-600'}`}>
                        {o.paymentStatus === 'FULLY_PAID' && <CheckCircle2 size={12} />}
                        {o.paymentStatus === 'PENDING' && <Clock size={12} />}
