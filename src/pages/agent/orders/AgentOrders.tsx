...
import {
  ...
  FileText, Image, PenTool, Clipboard, ImagePlus
} from 'lucide-react';
...
export default function AgentOrders() {
  ...
  const fetchMyOrders = async () => {
    setLoading(true);
    try {
      const data = await orderService.getMyOrders(page, 15);
      setOrders(data.content ?? []);
      setTotalPages(data.totalPages ?? 0);
    } catch (err: any) {
      toast.error(err?.response?.data?.message || err?.message || 'Failed to fetch your orders');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUploadLogo = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    const toastId = toast.loading('Uploading logo to order...');
    try {
      const files = Array.from(e.target.files);
      const updated = await orderService.uploadLogos(selectedOrder.id, files);
      setSelectedOrder(updated);
      setOrders(orders.map(o => o.id === selectedOrder.id ? updated : o));
      toast.success('Logo uploaded successfully', { id: toastId });
    } catch (err: any) {
      toast.error(err?.response?.data?.message || err?.message || 'Failed to upload logo', { id: toastId });
    } finally {
      setUploading(false);
    }
  };
  ...
  const handleUploadReferences = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (!selectedOrder || !e.target.files || e.target.files.length === 0) return;
    setUploading(true);
    const toastId = toast.loading('Uploading references to order...');
    try {
      const files = Array.from(e.target.files);
      const updated = await orderService.uploadReferences(selectedOrder.id, files);
      setSelectedOrder(updated);
      setOrders(orders.map(o => o.id === selectedOrder.id ? updated : o));
      toast.success('References uploaded successfully', { id: toastId });
    } catch (err: any) {
      toast.error(err?.response?.data?.message || err?.message || 'Failed to upload references', { id: toastId });
    } finally {
      setUploading(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-500 text-sm mt-1">Track progress of your submitted orders and copy public tracking links for clients.</p>
      </div>
                ...
                {filteredOrders.map(o => (
                  <tr key={o.id} className="hover:bg-gray-50 dark:hover:bg-gray-800/30 transition-colors">
                    ...
                    <td className="px-5 py-4 text-right flex justify-end gap-2">
                      <button
                        onClick={() => copyTrackingLink(o.trackingToken)}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
            ...
            >
              ...
              <div className="flex items-start justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-800 shrink-0">
                <div>
                  <p className="text-xs text-gray-400 font-mono">My Order Info</p>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 scrollbar-thin">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2.5 flex items-center gap-1.5">
                      <Layers size={13} /> Order Status & Tracking
                    </h3>
                    <div className="space-y-2 bg-gray-50 dark:bg-gray-800/30 p-3 rounded-xl border border-gray-100 dark:border-gray-800 text-xs">
                      <p><span className="text-gray-400">Current Phase:</span> <strong className="text-primary-500">{statusLabels[selectedOrder.status]}</strong></p>
                      <p><span className="text-gray-400">Designer Assigned:</span> {selectedOrder.designer ? selectedOrder.designer.name : 'Waiting for delegation'}</p>
                      <div className="pt-1 flex gap-2">
                        <button
                          onClick={() => copyTrackingLink(selectedOrder.trackingToken)}
                          className="px-2 py-1.5 bg-primary-50 dark:bg-primary-900/20 text-primary-500 font-bold border border-primary-100 dark:border-primary-800/50 rounded-lg flex items-center gap-1 hover:bg-primary-500 hover:text-white transition-colors"
                        >
                ...
                {/* Upload Section & Existing Assets */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {/* Logos & References */}
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2 p-3 bg-gray-50 dark:bg-gray-800/30 rounded-xl border border-gray-100 dark:border-gray-800">
                      ...
                      <button
                        ...
                        disabled={uploading}
                        className="w-14 h-14 rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600 hover:border-primary-500 hover:bg-primary-50 dark:hover:bg-primary-900/10 flex flex-col items-center justify-center gap-0.5 text-gray-400 hover:text-primary-500 transition-all shrink-0 cursor-pointer"
                        title="Upload logo"
                      >
                        <ImagePlus size={16} />
                      ...
                      {/* Render existing logos & references */}
                  ...
                  <div>
                    ...
                    {selectedOrder.mockupFileUrls?.length > 0 ? (
                      ...
                      <p className="text-xs text-gray-400 italic">No mockups submitted yet by the design team.</p>
                    )}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="ml-auto w-full max-w-sm space-y-2 text-xs">
                    ...
                    <div className="flex justify-between"><span className="text-gray-500">GST (18%)</span><span className="font-semibold">₹{selectedOrder.gstAmount.toLocaleString('en-IN')}</span></div>
